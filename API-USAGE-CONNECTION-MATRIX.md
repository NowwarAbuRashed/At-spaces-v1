...
## Meaning
- `Yes (live verified)`: confirmed by latest readiness runs and/or successful backend requests in `backend/backend-fix.log`.
- `Yes (frontend wired)`: connected in frontend API clients, but not directly proven by a successful 2xx readiness/live verification hit.
- `No`: in OpenAPI spec but not connected in current frontend API clients.

## Totals
- Total APIs in OpenAPI: **64**
- Currently in use (live verified): **17**
- Currently in use (frontend wired): **47**
- Not currently in use: **0**
- Connected from frontend: **63**
...
| GET /admin/approval-requests/{id} | Admin | Yes (frontend wired) | Approval request details (includes integrity hash) | Yes | admin-api.ts::getApprovalRequestDetails |
...
| GET /admin/audit-log | Admin | Yes (live verified) | Audit log (paged) | Yes | admin-api.ts::listAuditLog |
...
| POST /admin/auth/reset-password | Auth - Admin | Yes (frontend wired) | Admin reset password (requires TOTP) | Yes | auth-api.ts::adminResetPasswordRequest |
...
| POST /auth/customer/register-phone | Auth - Customer | Yes (live verified) | Register customer with phone (OTP) | Yes | customer-api.ts::customerRegisterPhoneRequest |
| POST /auth/customer/resend-otp | Auth - Customer | Yes (frontend wired) | Resend OTP to phone | Yes | customer-api.ts::customerResendOtpRequest |
| POST /auth/customer/reset-password | Auth - Customer | Yes (frontend wired) | Customer reset password | Yes | customer-api.ts::customerResetPasswordRequest |
| POST /auth/customer/verify-otp | Auth - Customer | Yes (frontend wired) | Verify OTP for customer | Yes | customer-api.ts::customerVerifyOtpRequest |
...
| POST /auth/vendor/reset-password | Auth - Vendor | Yes (frontend wired) | Vendor reset password | Yes | vendor-api.ts::vendorResetPasswordRequest |
...
| GET /features | Branches | Yes (live verified) | Features catalog (public) | Yes | customer-api.ts::listCustomerFeaturesRequest |
...
| GET /services/{id} | Services | Yes (frontend wired) | Get service details | Yes | customer-api.ts::getCustomerServiceDetailsRequest |
| POST /uploads/image | Uploads | Yes (live verified) | Upload image (vendor/admin) | Yes | uploads-api.ts::uploadImageRequest |
| GET /users/me | Users | Yes (live verified) | Get my profile | Yes | customer-api.ts::getCustomerProfileRequest, users-api.ts::getMe |
...
| POST /vendors/register | Vendors | Yes (frontend wired) | Vendor registration (requires admin approval) | Yes | vendor-api.ts::vendorRegisterRequest |
...
| GET /version | System | Yes (live verified) | API version | Yes | customer-api.ts::getBackendVersionRequest |
